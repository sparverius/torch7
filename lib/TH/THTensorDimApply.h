...
#define TH_TENSOR_DIM_APPLY3(TYPE1, TENSOR1, TYPE2, TENSOR2, TYPE3, TENSOR3,   \
                             DIMENSION, CODE)                                  \
  {                                                                            \
    ...
    if ((DIMENSION < 0) || (DIMENSION >= TENSOR1->nDimension))                 \
      THError("invalid dimension %d (expected to be 0 <= dim < %d)",           \
              DIMENSION, TENSOR1->nDimension);                                 \
    int same_dims = 1;                                                         \
    ...
    if (same_dims == 0) {                                                      \
      ...
      THError(                                                                 \
          "inconsistent tensor size, expected %s %s, %s %s and %s %s to have " \
          "the same "                                                          \
          "number of dimensions",                                              \
          #TENSOR1, T1buff.str, #TENSOR2, T2buff.str, #TENSOR3, T3buff.str);   \
    }                                                                          \
    int shape_check_flag = 0;                                                  \
    for (TH_TENSOR_DIM_APPLY_i = 0;                                            \
         TH_TENSOR_DIM_APPLY_i < TENSOR1->nDimension;                          \
         TH_TENSOR_DIM_APPLY_i++) {                                            \
    ...
    if (shape_check_flag == 1) {                                               \
      ...
      THDescBuff T3buff = _THSizeDesc(TENSOR3->size, TENSOR3->nDimension);     \
      THError(                                                                 \
          "Expected %s %s, %s %s and %s %s to have the same size in "          \
          "dimension %d",                                                      \
          #TENSOR1, T1buff.str, #TENSOR2, T2buff.str, #TENSOR3, T3buff.str,    \
          DIMENSION);                                                          \
    }                                                                          \
    ...
    TH_TENSOR_DIM_APPLY_counter =                                              \
        (long*)THAlloc(sizeof(long) * (TENSOR1->nDimension));                  \
    for (TH_TENSOR_DIM_APPLY_i = 0;                                            \
         TH_TENSOR_DIM_APPLY_i < TENSOR1->nDimension; TH_TENSOR_DIM_APPLY_i++) \
    ...
    while (!TH_TENSOR_DIM_APPLY_hasFinished) {                                 \
      ...
      for (TH_TENSOR_DIM_APPLY_i = 0;                                          \
        ...
        TENSOR3##_data += TENSOR3->stride[TH_TENSOR_DIM_APPLY_i];              \
                                                                               \
        if (TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] ==              \
            TENSOR1->size[TH_TENSOR_DIM_APPLY_i]) {                            \
          if (TH_TENSOR_DIM_APPLY_i == TENSOR1->nDimension - 1) {              \
            TH_TENSOR_DIM_APPLY_hasFinished = 1;                               \
            break;                                                             \
          } else {                                                             \
            TENSOR1##_data -=                                                  \
                TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] *           \
                TENSOR1->stride[TH_TENSOR_DIM_APPLY_i];                        \
            TENSOR2##_data -=                                                  \
                TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] *           \
                TENSOR2->stride[TH_TENSOR_DIM_APPLY_i];                        \
            TENSOR3##_data -=                                                  \
                TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] *           \
                TENSOR3->stride[TH_TENSOR_DIM_APPLY_i];                        \
            TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] = 0;            \
          }                                                                    \
        } else                                                                 \
          break;                                                               \
...
/**
 * Similar to DIM_APPLY(...) but we maintain two sets of pointers: one for the
 * first tensor and one for the second. The two tensors must have the same
 * shape, other than at the specified DIMENSION. This function makes it easy to
 * store the output from reducing the TENSOR at index. For example, in the sum
 * example described below, we could instead do:
 *
 * long i = 0;
 * TYPE1 sum;
 *
 ...
 *
 * In particular, we guarantee that the offset into TENSOR2 will be what you
 * would get if you applied all of the index values used to generate the offset
 * into TENSOR1.
 */
#define TH_TENSOR_DIM_APPLY2(TYPE1, TENSOR1, TYPE2, TENSOR2, DIMENSION, CODE)  \
  {                                                                            \
    ...
    if (TENSOR1->nDimension != TENSOR2->nDimension) {                          \
      ...
      THDescBuff T2buff = _THSizeDesc(TENSOR2->size, TENSOR2->nDimension);     \
      THError(                                                                 \
          "inconsistent tensor size, expected %s %s and %s %s to have the "    \
          "same "                                                              \
          "number of dimensions",                                              \
    ...
    for (TH_TENSOR_DIM_APPLY_i = 0;                                            \
      ...
      if (TENSOR1->size[TH_TENSOR_DIM_APPLY_i] !=                              \
        ...
        THDescBuff T2buff = _THSizeDesc(TENSOR2->size, TENSOR2->nDimension);   \
        THError(                                                               \
            "Expected %s %s and %s %s to have the same size in dimension %d",  \
...
/**
 * The basic idea for DIM_APPLY: Given a TENSOR and a DIMENSION, provide access
 * to the data stored at all sets of dimension values other than DIMENSION, such
 * that we can get all the values at those fixed indices for the various values
 * at DIMENSION.
 *
 * Suppose we have a 2x3x4 Tensor A, and we have DIMENSION=2. Then we will hit
 * CODE (2x3) times, and the pointer into storage will be at:
 *
 * A[0][0]
 * A[0][1]
 * A[0][2]
 ...
 * A[1][2]
 *
 * And at each point, we can access the data for each of the four elements of
 * the Tensor via TENSOR##_stride. So for example, if we wanted to sum the
 * elements there, we could do:
 *
 * long i = 0;
 * TYPE sum;
 ...
 * }
 *
 * Note that we don't have to have DIMENSION be the last tensor. If we have
 * DIMENSION=1, then we will hit the code (2x4) times, with pointer into the
 * storage at:
 *
 * offset +
 ...
 *
 * So we can again sum over the values at DIMENSION with the other indices
 * fixed.
 */
#define TH_TENSOR_DIM_APPLY(TYPE, TENSOR, DIMENSION, CODE)                    \
  {                                                                           \
    ...
    TH_TENSOR_DIM_APPLY_counter =                                             \
        (long*)THAlloc(sizeof(long) * (TENSOR->nDimension));                  \
    for (TH_TENSOR_DIM_APPLY_i = 0;                                           \
         TH_TENSOR_DIM_APPLY_i < TENSOR->nDimension; TH_TENSOR_DIM_APPLY_i++) \
      TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] = 0;                 \
                                                                              \
    while (!TH_TENSOR_DIM_APPLY_hasFinished) {                                \
      CODE                                                                    \
          ...
          if (TENSOR->nDimension == 1) break;                                 \
                                                                              \
      for (TH_TENSOR_DIM_APPLY_i = 0;                                         \
           TH_TENSOR_DIM_APPLY_i < TENSOR->nDimension;                        \
           TH_TENSOR_DIM_APPLY_i++) {                                         \
        /* Check if the index is equal to DIMENSION. We don't need to update  \
         * the */                                                             \
        /* offset if this is the case, and can consider the next index.       \
         * However, */                                                        \
        /* in the case that the DIMENSION is the last index in the Tensor,    \
         * then */                                                            \
        ...
        TENSOR##_data += TENSOR->stride[TH_TENSOR_DIM_APPLY_i];               \
                                                                              \
        if (TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] ==             \
            TENSOR->size[TH_TENSOR_DIM_APPLY_i]) {                            \
          /* Handled TENSOR_size(dim) iterations for DIM_APPLY_i. If this is  \
           * the last dimension, exit */                                      \
          if (TH_TENSOR_DIM_APPLY_i == TENSOR->nDimension - 1) {              \
            TH_TENSOR_DIM_APPLY_hasFinished = 1;                              \
            break;                                                            \
          } else {                                                            \
            /* Reset the counter, and the pointer to the beginning of the     \
             * storage for this combination of indices */                     \
            TENSOR##_data -=                                                  \
                TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] *          \
                TENSOR->stride[TH_TENSOR_DIM_APPLY_i];                        \
            TH_TENSOR_DIM_APPLY_counter[TH_TENSOR_DIM_APPLY_i] = 0;           \
          }                                                                   \
        } else                                                                \
          break;                                                              \
