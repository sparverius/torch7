...
#define __TH_TENSOR_APPLYX_PREAMBLE(TYPE, TENSOR, DIM, ALLOW_CONTIGUOUS)       \
  ...
  long *TENSOR##_counter = NULL, *TENSOR##_sizes = NULL,                       \
  ...
  if (TENSOR->nDimension == 0)                                                 \
    ...
    if (!TENSOR##_contiguous) {                                                \
      ...
      TENSOR##_dim = 1;                                                        \
      for (TENSOR##_i = TENSOR->nDimension - 2; TENSOR##_i >= 0;               \
           TENSOR##_i--) {                                                     \
        if (TENSOR->stride[TENSOR##_i] != TENSOR->stride[TENSOR##_i + 1] *     \
      ...
      TENSOR##_dimOffset = (DIM == TENSOR->nDimension - 1)                     \
                               ? &TENSOR##_i                                   \
                               : &TENSOR##_counter[DIM];                       \
      TENSOR##_sizes[TH_TENSOR_dim_index] =                                    \
          TENSOR->size[TENSOR->nDimension - 1];                                \
      TENSOR##_strides[TH_TENSOR_dim_index] =                                  \
          TENSOR->stride[TENSOR->nDimension - 1];                              \
...
#define __TH_TENSOR_APPLYX_UPDATE_COUNTERS(TENSOR, ALWAYS_UPDATE)            \
  if (TENSOR##_i == TENSOR##_size || ALWAYS_UPDATE) {                        \
    if (TENSOR##_contiguous) break;                                          \
                                                                             \
    if (TENSOR##_dim == 1) break;                                            \
                                                                             \
...
#define TH_TENSOR_APPLY3_D(TYPE1, TENSOR1, TYPE2, TENSOR2, TYPE3, TENSOR3,     \
                           DIM, CODE)                                          \
  {                                                                            \
    ...
    while (!TH_TENSOR_APPLY_hasFinished) {                                     \
      ...
      for (; TENSOR1##_i < TENSOR1##_size && TENSOR2##_i < TENSOR2##_size &&   \
           ...
           TENSOR1##_i++, TENSOR2##_i++, TENSOR3##_i++,                        \
    ...
    if (TENSOR1##_counter != NULL) THFree(TENSOR1##_counter);                  \
    if (TENSOR2##_counter != NULL) THFree(TENSOR2##_counter);                  \
    if (TENSOR3##_counter != NULL) THFree(TENSOR3##_counter);                  \
  }
  ...
  {                                                                          \
    ...
    if (TENSOR1##_n != TENSOR2##_n) {                                        \
      ...
      THError(                                                               \
          ...
          #TENSOR1, T1buff.str, #TENSOR2, T2buff.str, TENSOR1##_n,           \
          TENSOR2##_n);                                                      \
    }                                                                        \
    while (!TH_TENSOR_APPLY_hasFinished) {                                   \
      /* Loop through the inner most region of the Tensor */                 \
      for (; TENSOR1##_i < TENSOR1##_size && TENSOR2##_i < TENSOR2##_size;   \
           TENSOR1##_i++, TENSOR2##_i++, TENSOR1##_data += TENSOR1##_stride, \
