...
static const ptrdiff_t heapMaxDelta =
    (ptrdiff_t)1e6;  // limit to +/- 1MB before updating heapSize
static const ptrdiff_t heapMinDelta = (ptrdiff_t)-1e6;
static __thread ptrdiff_t heapSoftmax =
    (ptrdiff_t)3e8;  // 300MB, adjusted upward dynamically
...
static ptrdiff_t applyHeapDelta() {
  ...
  if (heapDelta > 0 && oldHeapSize > PTRDIFF_MAX - heapDelta)
    THError(
        "applyHeapDelta: heapSize(%td) + increased(%td) > PTRDIFF_MAX, "
        "heapSize overflow!",
        oldHeapSize, heapDelta);
  if (heapDelta < 0 && oldHeapSize < PTRDIFF_MIN - heapDelta)
    THError(
        "applyHeapDelta: heapSize(%td) + decreased(%td) < PTRDIFF_MIN, "
        "heapSize underflow!",
        oldHeapSize, heapDelta);
#endif
...
void THHeapUpdate(ptrdiff_t size) {
  ...
  if (size > 0 && heapDelta > PTRDIFF_MAX - size)
    THError(
        "THHeapUpdate: heapDelta(%td) + increased(%td) > PTRDIFF_MAX, "
        "heapDelta overflow!",
        heapDelta, size);
  if (size < 0 && heapDelta < PTRDIFF_MIN - size)
    THError(
        "THHeapUpdate: heapDelta(%td) + decreased(%td) < PTRDIFF_MIN, "
        "heapDelta underflow!",
        heapDelta, size);
#endif
...
static void* THAllocInternal(ptrdiff_t size) {
  ...
  if (size > 5120) {
    ...
    (!defined(DISABLE_POSIX_MEMALIGN))
...
void* THAlloc(ptrdiff_t size) {
  ...
  if (size < 0) THError("$ Torch: invalid memory size -- maybe an overflow?");
