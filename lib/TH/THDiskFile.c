...
size_t fread__(void* ptr, size_t size, size_t nitems, FILE* stream) {
  ...
  while (!feof(stream) && !ferror(stream) && (nread < nitems))
    nread += fread((char*)ptr + nread * size, size,
                   THMin(2147483648 / size, nitems - nread), stream);
  return nread;
}
#else
...
#define READ_WRITE_METHODS(TYPE, TYPEC, ASCII_READ_ELEM, ASCII_WRITE_ELEM)    \
  static size_t THDiskFile_read##TYPEC(THFile* self, TYPE* data, size_t n) {  \
    ...
    THArgCheck(dfself->file.isReadable, 1,                                    \
  ...
  static size_t THDiskFile_write##TYPEC(THFile* self, TYPE* data, size_t n) { \
    ...
    size_t nwrite = 0L;                                                       \
                                                                              \
    THArgCheck(dfself->handle != NULL, 1, "attempt to use a closed file");    \
    THArgCheck(dfself->file.isWritable, 1,                                    \
...
#if defined(_WIN64)
  THArgCheck(position <= (size_t)INT64_MAX, 2,
             "position must be smaller than INT64_MAX");
  if (_fseeki64(dfself->handle, (__int64)position, SEEK_SET) < 0)
#elif defined(_WIN32)
  THArgCheck(position <= (size_t)LONG_MAX, 2,
             "position must be smaller than LONG_MAX");
  if (fseek(dfself->handle, (long)position, SEEK_SET) < 0)
#else
  THArgCheck(position <= (size_t)LLONG_MAX, 2,
             "position must be smaller than LLONG_MAX");
...
static void THDiskFile_seekEnd(THFile* self) {
  ...
  {
    dfself->file.hasError = 1;
    if (!dfself->file.isQuiet) THError("unable to seek at end of file");
  }
}
...
/*                    int value = 0; int ret = fscanf(file->handle, "%d",
 * &value); array[i] = (value ? 1 : 0); if(ret <= 0) break; else result++, */
/*                    int value = (array[i] ? 1 : 0); nElemWritten =
...
READ_WRITE_METHODS(short, Short,
                   ...
                   if (ret <= 0) break; else nwrite++)

READ_WRITE_METHODS(int, Int, int ret = fscanf(dfself->handle, "%d", &data[i]);
                   if (ret <= 0) break;
                   else nread++,
...
READ_WRITE_METHODS(
    THHalf, Half, float buf; int ret = fscanf(dfself->handle, "%g", &buf);
    if (ret <= 0) break;
    else {
...
/* For Long we need to rewrite everything, because of the special management of
 * longSize */
...
THFile* THDiskFile_new(const char* name, const char* mode, int isQuiet) {
  ...
  THArgCheck(THDiskFile_mode(mode, &isReadable, &isWritable), 2,
  ...
  if (!handle) {
    if (isQuiet)
      ...
      THError("cannot open <%s> in mode %c%c", name, (isReadable ? 'r' : ' '),
              (isWritable ? 'w' : ' '));
  }
...
THFile* THPipeFile_new(const char* name, const char* mode, int isQuiet) {
  ...
  THArgCheck(THPipeFile_mode(mode, &isReadable, &isWritable), 2,
