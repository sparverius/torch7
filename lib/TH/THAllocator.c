...
THMapAllocatorContext* THMapAllocatorContext_new(const char* filename,
  ...
  if (!(flags & TH_ALLOCATOR_MAPPED_SHARED) &&
      !(flags & TH_ALLOCATOR_MAPPED_SHAREDMEM))
...
static void* _map_alloc(void* ctx_, ptrdiff_t size) {
...
#ifdef _WIN32
  {
    ...
    if (ctx->flags) {
      hfile = CreateFileA(ctx->filename, GENERIC_READ | GENERIC_WRITE,
                          FILE_SHARE_WRITE | FILE_SHARE_READ, 0, OPEN_ALWAYS,
                          FILE_ATTRIBUTE_NORMAL, 0);
      if (hfile == INVALID_HANDLE_VALUE)
        THError("could not open file <%s> in read-write mode; error code: <%d>",
                ctx->filename, GetLastError());
    } else {
      hfile = CreateFileA(ctx->filename, GENERIC_READ,
                          FILE_SHARE_WRITE | FILE_SHARE_READ, 0, OPEN_EXISTING,
                          FILE_ATTRIBUTE_NORMAL, 0);
      if (hfile == INVALID_HANDLE_VALUE)
        THError("could not open file <%s> in read-only mode; error code: <%d>",
                ctx->filename, GetLastError());
    }

    if (GetFileSizeEx(hfile, &hfilesz) == 0) {
      THError("could not get file size: <%s>; error code: <%d>", ctx->filename,
              GetLastError());
    }
      ...
      if (size > hfilesz.QuadPart) {
        if (ctx->flags) {
          ...
          if (SetFilePointerEx(hfile, hfilesz, NULL, FILE_BEGIN) == 0) {
            CloseHandle(hfile);
            THError(
                "unable to stretch file <%s> to the right size; error code: "
                "<%d>",
                ctx->filename, GetLastError());
          }
          if (SetEndOfFile(hfile) == 0) {
            CloseHandle(hfile);
            THError("unable to write to file <%s>; error code: <%d>",
                    ctx->filename, GetLastError());
          }
        } else {
          CloseHandle(hfile);
          THError(
              ...
              "error code: <%d>",
              ctx->filename, size, GetLastError());
        }
      }
    } else
    ...
    if (ctx->flags) {
      if ((hmfile =
               CreateFileMapping(hfile, NULL, PAGE_READWRITE, hfilesz.HighPart,
                                 hfilesz.LowPart, NULL)) == NULL)
        THError("could not create a map on file <%s>; error code: <%d>",
                ctx->filename, GetLastError());
    } else {
      if ((hmfile =
               CreateFileMapping(hfile, NULL, PAGE_WRITECOPY, hfilesz.HighPart,
                                 hfilesz.LowPart, NULL)) == NULL)
        THError("could not create a map on file <%s>; error code: <%d>",
                ctx->filename, GetLastError());
    }
...
#else /* _WIN32 */
  {
    ...
    if (ctx->flags &
        (TH_ALLOCATOR_MAPPED_SHARED | TH_ALLOCATOR_MAPPED_SHAREDMEM))
      flags = O_RDWR | O_CREAT;
    else
      flags = O_RDONLY;

    if (ctx->flags & TH_ALLOCATOR_MAPPED_EXCLUSIVE) flags |= O_EXCL;
    if (ctx->flags & TH_ALLOCATOR_MAPPED_NOCREATE) flags &= ~O_CREAT;
    ...
    if (!(ctx->flags & TH_ALLOCATOR_MAPPED_FROMFD)) {
      if (ctx->flags & TH_ALLOCATOR_MAPPED_SHARED) {
        ...
        if ((fd = shm_open(ctx->filename, flags, (mode_t)0600)) == -1)
          THError("unable to open shared memory object <%s> in read-write mode",
                  ctx->filename);
#else
        THError(
    ...
    if (fstat(fd, &file_stat) == -1) {
      if (!(ctx->flags & TH_ALLOCATOR_MAPPED_FROMFD)) close(fd);
      ...
      if (size > file_stat.st_size) {
        if (ctx->flags) {
          if (ftruncate(fd, size) == -1)
            THError("unable to resize file <%s> to the right size",
                    ctx->filename);
          if (fstat(fd, &file_stat) == -1 || file_stat.st_size < size) {
            close(fd);
            THError("unable to stretch file <%s> to the right size",
                    ctx->filename);
          }
 ...
 */
    ...
    if (ctx->flags & TH_ALLOCATOR_MAPPED_KEEPFD) {
      ...
      if (close(fd) == -1) THError("Error closing file <%s>", ctx->filename);
      ctx->fd = -1;
    }
    ...
    if (data == MAP_FAILED) {
      ...
      THError("$ Torch: unable to mmap memory: you tried to mmap %dGB.",
              ctx->size / 1073741824);
    }
  }
#endif
...
static void THMapAllocator_free(void* ctx_, void* data) {
  ...
  if (!(ctx->flags &
        (TH_ALLOCATOR_MAPPED_FROMFD | TH_ALLOCATOR_MAPPED_UNLINK))) {
    if (ctx->flags & TH_ALLOCATOR_MAPPED_SHAREDMEM) {
#ifdef HAVE_SHM_UNLINK
      if (shm_unlink(ctx->filename) == -1)
        THError("could not unlink the shared memory file %s", ctx->filename);
...
static void* THRefcountedMapAllocator_alloc(void* _ctx, ptrdiff_t size) {
  ...
  if (ctx->flags & TH_ALLOCATOR_MAPPED_FROMFD)
    THError(
        "THRefcountedMapAllocator doesn't support TH_ALLOCATOR_MAPPED_FROMFD "
        "flag");
  if (ctx->flags & TH_ALLOCATOR_MAPPED_KEEPFD)
    THError(
        "THRefcountedMapAllocator doesn't support TH_ALLOCATOR_MAPPED_KEEPFD "
        "flag");
  if (ctx->flags & TH_ALLOCATOR_MAPPED_UNLINK)
    THError(
        "THRefcountedMapAllocator doesn't support TH_ALLOCATOR_MAPPED_UNLINK "
        "flag");
  if (!(ctx->flags & TH_ALLOCATOR_MAPPED_SHAREDMEM))
    THError(
        "THRefcountedMapAllocator requires TH_ALLOCATOR_MAPPED_SHAREDMEM flag");
...
#else
    THError(
        "could not unlink the shared memory file %s, shm_unlink not available "
        "on platform",
        ctx->filename);
