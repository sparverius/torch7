...
#define READ_WRITE_METHODS(TYPE, TYPEC, ASCII_READ_ELEM, ASCII_WRITE_ELEM,     \
  ...
  static size_t THMemoryFile_read##TYPEC(THFile* self, TYPE* data, size_t n) { \
    ...
    THArgCheck(mfself->file.isReadable, 1,                                     \
      ...
      if (mfself->file.isAutoSpacing && (n > 0)) {                             \
        if ((mfself->position < mfself->size) &&                               \
          ...
          mfself->position++;                                                  \
                                                                               ...
                                                                               \
    if (nread != n) {                                                          \
      mfself->file.hasError =                                                  \
          1; /* shouldn't we put hasError to 0 all the time ? */               \
    ...
    return nread;                                                              \
                                                                               ...
                                                                               \
  static size_t THMemoryFile_write##TYPEC(THFile* self, TYPE* data,            \
                                          size_t n) {                          \
    THMemoryFile* mfself = (THMemoryFile*)self;                                \
                                                                               \
    THArgCheck(mfself->storage != NULL, 1, "attempt to use a closed file");    \
    THArgCheck(mfself->file.isWritable, 1,                                     \
    ...
    if (mfself->file.isBinary) {                                               \
      ...
      for (i = 0; i < n; i++) {                                                \
        ...
        while (1) {                                                            \
          ...
          THMemoryFile_grow(mfself, mfself->storage->size +                    \
                                        (mfself->storage->size / 2) + 2);      \
        }                                                                      \
...
void THMemoryFile_longSize(THFile* self, int size) {
  ...
  THArgCheck(size == 0 || size == 4 || size == 8, 1,
...
/*                    int value = 0; int ret =
 * sscanf(mfself->storage->data+mfself->position, "%d%n", &value, &nByteRead);
 * data[i] = (value ? 1 : 0), */
/*                    int value = (data[i] ? 1 : 0); nByteWritten =
 * snprintf(mfself->storage->data+mfself->position,
 * mfself->storage->size-mfself->position, "%d", value), */
/*                    1) */
...
READ_WRITE_METHODS(
    ...
    size_t ret = (mfself->position + n <= mfself->size
                      ? n
                      : mfself->size - mfself->position);
    if (spacePtr) * spacePtr = spaceChar; nByteRead = ret; nread = ret;
    i = n - 1;
...
/* DEBUG: we should check if %n is count or not as a element (so ret might need
 * to be ret-- on some systems) */
...
READ_WRITE_METHODS(
    short, Short, int nByteRead_;
    int ret = sscanf(mfself->storage->data + mfself->position, "%hd%n",
                     &data[i], &nByteRead_);
    nByteRead = nByteRead_; if (ret <= 0) break;
    else nread++,
    nByteWritten = snprintf(mfself->storage->data + mfself->position,
                            mfself->storage->size - mfself->position, "%hd",
                            data[i]),
    1)

READ_WRITE_METHODS(int, Int, int nByteRead_;
                   int ret = sscanf(mfself->storage->data + mfself->position,
                                    "%d%n", &data[i], &nByteRead_);
                   nByteRead = nByteRead_; if (ret <= 0) break;
                   else nread++,
                   nByteWritten = snprintf(
                       mfself->storage->data + mfself->position,
                       mfself->storage->size - mfself->position, "%d", data[i]),
                   1)

READ_WRITE_METHODS(
    float, Float, int nByteRead_;
    int ret = sscanf(mfself->storage->data + mfself->position, "%g%n", &data[i],
                     &nByteRead_);
    nByteRead = nByteRead_; if (ret <= 0) break;
    else nread++,
    nByteWritten = snprintf(mfself->storage->data + mfself->position,
                            mfself->storage->size - mfself->position, "%.9g",
                            data[i]),
    1)

READ_WRITE_METHODS(
    THHalf, Half, int nByteRead_; float buf;
    int ret = sscanf(mfself->storage->data + mfself->position, "%g%n", &buf,
                     &nByteRead_);
    data[i] = TH_float2half(buf); nByteRead = nByteRead_; if (ret <= 0) break;
    else nread++,
    nByteWritten = snprintf(mfself->storage->data + mfself->position,
                            mfself->storage->size - mfself->position, "%.9g",
                            TH_half2float(data[i])),
    1)

READ_WRITE_METHODS(
    double, Double, int nByteRead_;
    int ret = sscanf(mfself->storage->data + mfself->position, "%lg%n",
                     &data[i], &nByteRead_);
    nByteRead = nByteRead_; if (ret <= 0) break;
    else nread++,
    nByteWritten = snprintf(mfself->storage->data + mfself->position,
                            mfself->storage->size - mfself->position, "%.17g",
                            data[i]),
    1)
...
THFile* THMemoryFile_newWithStorage(THCharStorage* storage, const char* mode) {
  ...
  if (storage) {
    THArgCheck(storage->data[storage->size - 1] == '\0', 1,
               "provided CharStorage must be terminated by 0");
    THArgCheck(THMemoryFile_mode(mode, &isReadable, &isWritable), 2,
